...
function processDir(dir) {
    ...
    for (const file of files) {
        ...
        if (fs.statSync(fullPath).isDirectory()) {
            ...
            content = content.replace(/\bbg-zinc-800\b/g, 'bg-surface');

            // Surfaces and cards
            content = content.replace(/\bbg-white\/5\b/g, 'bg-card-bg');
            content = content.replace(/\bhover:bg-white\/5\b/g, 'hover:bg-card-hover-bg');
            content = content.replace(/\bhover:bg-white\/10\b/g, 'hover:bg-card-hover-bg');
            content = content.replace(/\bbg-white\/\[0\.02\]\b/g, 'bg-card-bg');
            ...
            content = content.replace(/\bbg-white\/20\b/g, 'bg-card-border');
            ...
            content = content.replace(/\bborder-white\/20\b/g, 'border-card-border');
            ...
            content = content.replace(/(bg-[a-z]+-[0-9]+.*?)text-foreground/g, '$1text-white');
            content = content.replace(/text-foreground(.*?)bg-[a-z]+-[0-9]+/g, 'text-white$1bg-indigo-600');
...
processDir('e:/Sahin Alom/sahin-alom/slices');
