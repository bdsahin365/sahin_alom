...
export default function HeaderLogo({ compact = false, showSubtitle = true, onClick }: Props) {
  ...
  const customLogo = B.logo
  const showEmblem = B.showLogoEmblem !== false
  const showBrandTitle = B.showBrandTitle !== false
  const brandTitleText = B.brandTitle || E.initials || 'SAHIN ALOM'
  const showBadge = B.showCredentialBadge !== false && (B.credentialBadge || E.credentialsTag)
  const badgeText = B.credentialBadge || E.credentialsTag || 'PE'
  const showSub = showSubtitle && B.showBrandSubtitle !== false
  const subtitleText = B.brandSubtitle || (E.title ? `${E.title.toUpperCase()} • ABC LICENSED` : 'ELECTRICAL ENGINEER • ABC LICENSED')

  // If everything is hidden, fallback gracefully
  const hasText = showBrandTitle || showBadge || showSub
  ...
  return (
    ...
    >
      {/* 1. Custom Image / SVG Logo or Precision Vector CAD Emblem */}
      {showEmblem && (
        customLogo ? (
          <div
            style={{
      ...
      {/* 2. Brand Name, Credential Badge & Subtitle Typography */}
      {hasText && (
